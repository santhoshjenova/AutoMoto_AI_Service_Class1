...
st.markdown("Personalised, AI-powered vehicle service prediction and message generator for dealerships.")

st.header("Enter Customer & Vehicle Details")

# Input features; ensure these match your model's expected features/order
...
# Prepare input dictionary matching your model's feature input
feature_dict = {
...
if st.button("Predict & Generate Reminder"):
    ...
    # Generate dynamic message based on prediction and feedback score
    if prediction == 1:
        if feedback_score <= 2:
            msg = (
